...
public class WebshopUserRepository(AccountsDbContext dbContext, UserManager<WebshopIdentityUser> userManager, IEnumerable<IFilteredQueryBuilder<WebshopIdentityUser, string, WebshopUserSearchObject>> queryFilters, IMapper mapper)
    : IEntityRepository<WebshopUserEntity, string, WebshopUserSearchObject, EntitySortBy, WebshopUserIncludes>
{
    ...
    {
        IQueryable<WebshopIdentityUser> query = Query(dbContext.Users, searchObjects, includes, pagingInfo);
    ...
    public Task<WebshopIdentityUser?> GetItem(string id)
    {
        return AddIncludes(dbContext.Users, WebshopUserIncludes.All)
            .AsNoTrackingWithIdentityResolution()
            .FirstOrDefaultAsync(x => x.Id == id);
    }
    public IQueryable<WebshopIdentityUser> Filter(IQueryable<WebshopIdentityUser> query, WebshopUserSearchObject? so)
    {
    ...
    public IQueryable<WebshopIdentityUser> Filter(IQueryable<WebshopIdentityUser> query, IList<WebshopUserSearchObject?> searchObjects)
        => searchObjects.Aggregate((IQueryable<WebshopIdentityUser>?)null, (r, so) => r == null ? Filter(query, so) : r.Union(Filter(query, so))) ?? query;
    public IQueryable<WebshopIdentityUser> AddIncludes(IQueryable<WebshopIdentityUser> query, WebshopUserIncludes? includes)
    {
        if (includes != null)
        {
    ...
    public virtual IQueryable<WebshopIdentityUser> Query(IQueryable<WebshopIdentityUser> query, IList<WebshopUserSearchObject?> searchObjects, WebshopUserIncludes? includes, PagingInfo? pagingInfo)
    {
    ...
    public async Task Add(WebshopUserEntity model)
    {
        ...
        var item = mapper.Map<WebshopIdentityUser>(model);
    ...
    public void PrepareItem(WebshopUserEntity model, WebshopIdentityUser? original)
    {
    ...
    public async Task UpdateUser(WebshopUserEntity model, WebshopIdentityUser item)
    {
    ...
    public Task Modify(WebshopUserEntity item, WebshopIdentityUser original)
    {
        var modified = mapper.Map<WebshopIdentityUser>(item);
